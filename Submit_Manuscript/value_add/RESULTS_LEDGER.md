...
| **CD55** | 5 | 1987.93 | −0.000185 (**0.741**) | **−0.002837 (7.5×10⁻⁶)** | **0.280** (no het.) | 0.801 (0.287/0.976 sens.) | **Robust MR, coloc prior-sensitive.** Clean sensitivity panel. = Qin-2024 target → not a differentiator. |
...
2. **CD55** has a genuinely clean MR sensitivity profile (no pleiotropy, no
   ...
   association* whose *colocalization is prior-sensitive*; concede it is a Qin-2024
...
| CD55 | anchor | **TRUE** | known locus → positive control (= Qin-2024 target) |
...
CD55 ∈ Qin-2024's six complement targets (CD55, CD46, CFH, C2, C3, CFB); **no other
...
## ⚠ PHASE-0 CORRECTIONS APPLIED (2026-08) — citation & gene identity

1. **Attribution corrected.** The paper cited throughout as "Wang et al. 2024" is
   **Qin Y, Lei C, Lin T, Han X, Wang D. Invest Ophthalmol Vis Sci 2024;65(10):13**
   (doi:10.1167/iovs.65.10.13, PMID 39110588) — Wang is the *last* author. All
   manuscript, ledger, table and legend text updated to "Qin et al. (2024)".
2. **Gene identity corrected.** The multi-omics study nominated **UBE2I**, not "UBE".
   The audit assigned `ENSG00000198954` to a non-standard symbol → that row is INVALID
   until re-run with UBE2I (ENSG00000103275). `GATS`/CASTOR3 also needs ID confirmation.
3. **PRMT6 contradiction OPEN.** `published_targets_audit.csv` has PRMT6 with n_snps=3,
   F=2465, MR P=1.9e-4 yet `coloc_status=not_evaluable (n_cis_snps=0)`. Both cannot be
   true — re-run required before submission.

**`outputs/published_targets_audit.csv` still carries the old "Wang 2024" source label
and the wrong UBE row; it is a script output and must be regenerated by the PI (Phase 0),
not hand-edited.**
...
(Qin et al. 2024 n=6; multi-omics 2024 n=6). Verified PP.H4:

| Gene | Src | PP.H4 | PP.H1 | Verdict |
|---|---|---|---|---|
| CD55 | Qin | **0.808** | 0.104 | **reproduced** (coloc>0.8 + CREAM p=1.5e-7) |
| TSSK6 | Qin | 0.782 | 0.045 | moderate coloc, NO replication; testis-restricted, GATAD2A cluster → caveat |
| SH3YL1 | Multi | 0.748 | 0.167 | moderate coloc, no replication |
| WNT3 | Qin | 0.119 | **0.861** | distinct variant (fail) |
| ZKSCAN4 | Multi | 0.021 | **0.916** | distinct variant (fail) |
| BTN3A1 | Qin | 0.002 | **0.968** | distinct variant (fail) |
| LCAT | Qin | 0.0004 | **0.957** | distinct variant (fail) |
| NPAT | Multi | 0.0003 | 0.203 | fail (ambiguous/low) |
| CD34 | Qin | 0.0003 | **0.787** | distinct variant (fail) |
| PRMT6 | Multi | NA | NA | not evaluable — 0 blood cis-eQTL |
| GATS | Multi | NA | NA | not evaluable — 0 blood cis-eQTL |
| UBE2I | Multi | NA | NA | not evaluable — 0 blood cis-eQTL |
...
- TISSUE: Qin et al. used blood + retina eQTL; our audit is blood-only. Blood failure ≠
...
- CD55 (only survivor) is not novel (= Qin et al. 2024 target). Contribution is the NEGATIVE result.
